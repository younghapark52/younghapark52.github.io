...
홀수 = 짝수 + 1이므로, 결국 마지막 1을 누가 가져가느냐의 싸움으로 이해할 수 있습니다.
...
따라서 p[i-1]->pre[i-1] 간선이 존재하고, suf[i]->p[i] 간선이 존재합니다.

그리고 모든 i에 대해 pre[i-1]->suf[i] 간선은 존재합니다.
...
이번 div3 출제자가 **토키와다이의 레일건**이었는데, 즐겁게 참여했습니다. (어과초 매우 재밌게 보는 중)
